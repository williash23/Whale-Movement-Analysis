# Sara Williams
# 9/18/2019; 2/2/2020; 10/23/2020
# Data prep 
################################################################################

#  Load packages
library(dplyr)
library(adehabitatLT)    
library(tidyr)
library(stringr)
library(aspace)
library(sf)

`%!in%` = function(x,y) !(x %in% y)

################################################################################

#  Load data
dat_raw <- read.csv("C:/Users/saraw/Documents/Whales/data/Whales_0615_general_clean.csv")
sa <- st_read("C:/Users/saraw/Documents/Whales/Tracks_0615/survey_area_buff15k.shp")

#  Data clean up and manipulation
tmp1 <- dat_raw %>%
	dplyr::filter(year > 2009) %>% ## per info from Karin, PTB (bearing) was kind of weird before 2010
	dplyr::filter(!is.na(X_whale_UTM)) %>%
	group_by(same_whale_ID) %>%
	filter(n() > 1) %>%
	as.data.frame()  %>%
	arrange(same_whale_ID, ob_order_time) 

ids_only1count <- tmp1 %>% 
	group_by(same_whale_ID) %>%
	mutate(max_count = max(count)) %>%
	as.data.frame() %>%
	dplyr::filter(max_count < 2)
ids_only1count <- as.character(unique(ids_only1count$same_whale_ID))

tmp2 <- tmp1 %>%
	dplyr::filter(same_whale_ID %in% ids_only1count) 

tmp3 <- tmp2 %>% 
	tidyr::separate(TimeTxt, c("hrs", "mins", "sec"), sep = ":", remove = FALSE) %>%
	mutate_if(is.character, as.numeric) %>%
	mutate(time_sec = sec + mins*60 + hrs*60*60) %>%
	group_by(same_whale_ID) %>%
	mutate(tdiff_s = abs(time_sec - lead(time_sec))) %>%
	as.data.frame() 
	
tmp4a <- tmp3 %>%
	dplyr::select(-tdiff_s) %>%
	mutate(X = X_whale_UTM, Y = Y_whale_UTM) %>%
	st_as_sf(coords = c("X", "Y"), crs = 32608) %>%
	group_by(same_whale_ID) %>%
	mutate(lead_tmp = geometry[row_number() + 1]) %>%
    mutate(dist_m = as.numeric(st_distance(geometry, lead_tmp, by_element = T))) %>%
    mutate(tdiff_s = abs(time_sec - lead(time_sec))) %>%
	mutate(spd_mps = dist_m/tdiff_s) %>%
	as.data.frame() %>% 
	dplyr::select(unique_event_ID, same_whale_ID, count,
		X_whale_UTM, Y_whale_UTM, 
		dist_m, tdiff_s, spd_mps, time_sec, ob_order_time, 
		ship_whale_dist,  ship_whale_bearing, 
		whale_behavior, visibility, TimeTxt) 

too_fast1 <- tmp4a %>% dplyr::filter(spd_mps > 4.8) #7.5)		
rem_ids1 <- as.character(unique(too_fast1$unique_event_ID))
tmp4 <- tmp4a %>% dplyr::filter(unique_event_ID %!in% rem_ids1) 


tmp5a <- tmp4 %>%
	dplyr::select(unique_event_ID, same_whale_ID, count,
		X_whale_UTM, Y_whale_UTM, 
		time_sec, ob_order_time, 
		ship_whale_dist,  ship_whale_bearing, 
		whale_behavior, visibility, TimeTxt) %>%
	mutate(X = X_whale_UTM, Y = Y_whale_UTM) %>%
	st_as_sf(coords = c("X", "Y"), crs = 32608) %>%
	group_by(same_whale_ID) %>%
	filter(n() > 1) %>%
	mutate(lead_tmp = geometry[row_number() + 1]) %>%
    mutate(dist_m = as.numeric(st_distance(geometry, lead_tmp, by_element = T))) %>%
	mutate(tdiff_s = abs(time_sec - lead(time_sec))) %>%
	mutate(spd_mps = dist_m/tdiff_s) %>%
	as.data.frame() 

too_fast2 <- tmp5a %>% dplyr::filter(spd_mps > 4.8) #7.5)		
rem_ids2 <- as.character(unique(too_fast2$unique_event_ID))
tmp5 <- tmp5a %>% dplyr::filter(unique_event_ID %!in% rem_ids2) 


tmp6a <- tmp5 %>%
	dplyr::select(unique_event_ID, same_whale_ID, count,
		X_whale_UTM, Y_whale_UTM, 
		time_sec, ob_order_time, 
		ship_whale_dist,  ship_whale_bearing, 
		whale_behavior, visibility, TimeTxt) %>%
	mutate(X = X_whale_UTM, Y = Y_whale_UTM) %>%
	st_as_sf(coords = c("X", "Y"), crs = 32608) %>%
	group_by(same_whale_ID) %>%
	filter(n() > 1) %>%
	mutate(lead_tmp = geometry[row_number() + 1]) %>%
    mutate(dist_m = as.numeric(st_distance(geometry, lead_tmp, by_element = T))) %>%
	mutate(tdiff_s = abs(time_sec - lead(time_sec))) %>%
	mutate(spd_mps = dist_m/tdiff_s) %>%
	as.data.frame() 
	
too_fast3 <- tmp6a %>% dplyr::filter(spd_mps > 4.8) #7.5)		
rem_ids3 <- as.character(unique(too_fast3$unique_event_ID))
tmp6 <- tmp6a %>% dplyr::filter(unique_event_ID %!in% rem_ids3) 


tmp7a <- tmp6 %>%
	dplyr::select(unique_event_ID, same_whale_ID, count,
		X_whale_UTM, Y_whale_UTM, 
		time_sec, ob_order_time, 
		ship_whale_dist,  ship_whale_bearing, 
		whale_behavior, visibility, TimeTxt) %>%
	mutate(X = X_whale_UTM, Y = Y_whale_UTM) %>%
	st_as_sf(coords = c("X", "Y"), crs = 32608) %>%
	group_by(same_whale_ID) %>%
	filter(n() > 1) %>%
	mutate(lead_tmp = geometry[row_number() + 1]) %>%
    mutate(dist_m = as.numeric(st_distance(geometry, lead_tmp, by_element = T))) %>%
	mutate(tdiff_s = abs(time_sec - lead(time_sec))) %>%
	mutate(spd_mps = dist_m/tdiff_s) %>%
	as.data.frame() 
	
too_fast4 <- tmp7a %>% dplyr::filter(spd_mps > 4.8) #7.5)		
rem_ids4 <- as.character(unique(too_fast4$unique_event_ID))
tmp7 <- tmp7a %>% dplyr::filter(unique_event_ID %!in% rem_ids4) 

### All movements that are greater than 4.8 m/s removed by this point

tmp8 <- tmp7 %>%
	dplyr::select(unique_event_ID, same_whale_ID, count,
		X_whale_UTM, Y_whale_UTM, 
		time_sec, ob_order_time, 
		ship_whale_dist,  ship_whale_bearing, 
		whale_behavior, visibility, TimeTxt) %>%
	mutate(X = X_whale_UTM, Y = Y_whale_UTM) %>%
	st_as_sf(coords = c("X", "Y"), crs = 32608) %>%
	group_by(same_whale_ID) %>%
	filter(n() > 1) %>%
	mutate(lead_tmp = geometry[row_number() + 1]) %>%
    mutate(dist_m = as.numeric(st_distance(geometry, lead_tmp, by_element = T))) %>%
	mutate(tdiff_s = abs(time_sec - lead(time_sec))) %>%
	mutate(spd_mps = dist_m/tdiff_s) %>%
	as.data.frame() 
tmp8_sf <- tmp8 %>%
	mutate(X = X_whale_UTM, Y = Y_whale_UTM) %>%
	st_as_sf(coords = c("X", "Y"), crs = 32608) 


#  Create trajectory (ltraj) object for steps and turns
tmp8$same_whale_ID <- droplevels(tmp8$same_whale_ID)
tmp9 <- tmp8 %>%
	group_by(same_whale_ID) %>%
	mutate(new_id = cur_group_id()) %>%
	as.data.frame() %>%
	dplyr::select(X_whale_UTM, Y_whale_UTM, 
		ob_order_time, TimeTxt, tdiff_s, 
		ship_whale_dist, ship_whale_bearing, whale_behavior, 
		same_whale_ID, new_id, unique_event_ID) %>%
	arrange(same_whale_ID, ob_order_time) %>%
	mutate(unique_record = 1:n()) %>%
	mutate(sim_DateTime = paste("2015-08-01", TimeTxt, sep = " ")) 
tmp9$sim_DateTime <- lubridate::as_datetime(tmp9$sim_DateTime)


traj <- as.ltraj(xy = tmp9[,c("X_whale_UTM","Y_whale_UTM")], 
	id = tmp9$same_whale_ID, date = tmp9$sim_DateTime, typeII = TRUE)

traj_rd_1min <- redisltraj(traj, u = 60, type = "time")




#  Do each individual separateyl
ids <- as.character(unique(tmp9$same_whale_ID))
recs <- tmp9 %>%
	group_by(same_whale_ID) %>%
	slice(1) %>%
	as.data.frame()
recs <- recs$unique_record

ls <- list()
for(i in 1:length(ids)){
	nam <- ids[i]
	rec <- recs[i]
	nam_dat <- tmp9 %>% dplyr::filter(unique_record == recs[i])
	
	tmp1 <- tmp9 %>% 
		dplyr::filter(same_whale_ID == ids[i]) %>%
		dplyr::select(same_whale_ID, sim_DateTime, X_whale_UTM, Y_whale_UTM,
			ob_order_time, TimeTxt, tdiff_s, 
			ship_whale_dist, ship_whale_bearing, whale_behavior, unique_record)
	tmp1a <- tmp1[1,]
	tmp1$same_whale_ID <- droplevels(tmp1$same_whale_ID)
	tmp2 <- as.ltraj(xy = tmp1[,c("X_whale_UTM","Y_whale_UTM")], 
		id = tmp1$same_whale_ID, date = tmp1$sim_DateTime, typeII = TRUE)
	tmp3 <- redisltraj(tmp2, u = 120, type = "time")
	
	splt_n <- (nrow(tmp3[[1]]) - nrow(tmp2[[1]])) + 2
	sign <- ifelse(tmp1$ship_whale_dist[1] >= tmp1$ship_whale_dist[nrow(tmp1)], -1, 1)
	total_diff_dist <- sign * (tmp1$ship_whale_dist[1] - tmp1$ship_whale_dist[nrow(tmp1)])
	splt_diff_dist <- cumsum(rep(total_diff_dist/splt_n, splt_n))
	reg_ship_whale_dist <- c(tmp1$ship_whale_dist[1] , tmp1$ship_whale_dist[1] + splt_diff_dist)
	
	#  Convert into dataframe
	df <- ld(tmp3)
	df <- df %>%
		dplyr::select(id, x, y, dist, rel.angle, abs.angle, dx, dy, dt) 
	names(df) <- c("same_whale_ID", "X_whale_UTM", "Y_whale_UTM", 
		"step", "turn", "abs_angle", "dx", "dy", "dt")
	df <- df %>% 
		left_join(tmp1a, by = c("same_whale_ID")) %>%
		cbind(reg_ship_whale_dist)
		
	ls[[i]] <- df
	}


dat <- do.call(rbind, ls) %>%
	mutate(spd_mps = step/tdiff_s) %>%
	group_by(same_whale_ID) %>%
	dplyr::mutate(occ = 1:n()) %>%
	dplyr::mutate(n_occ = n()) %>%
	as.data.frame() %>%
	dplyr::filter(ship_whale_dist < 5001)




#  Format data for use in model with parameters estaimted per index
mod_dat <- dat %>%
	dplyr::filter(!is.na(step)) %>%
	mutate(bin100_ship_whale_dist = round(ship_whale_dist, -2)) %>%
	mutate(bin1000_ship_whale_dist = round(ship_whale_dist, -3)) %>%
	mutate(dist_ind = ifelse(ship_whale_dist < 1001, 1, 
		ifelse(ship_whale_dist >= 1001 & ship_whale_dist < 3001, 2, 3))) %>%
	mutate(bear_ind = ifelse(abs(ship_whale_bearing) < 22.56, 1, 
		ifelse(abs(ship_whale_bearing) >= 22.56 & abs(ship_whale_bearing) < 45, 2, 3))) %>%
	mutate(beh_ind = ifelse(whale_behavior == "BL-Blowing" | whale_behavior == "DF-Dive0fluke-up" |
		whale_behavior == "DN-Dive-no-fluke", 1, 2))  %>%
	mutate(comb_ind = ifelse(dist_ind == 1 & bear_ind == 1, 1, # close and front
		ifelse(dist_ind == 1 & bear_ind == 3, 2, # close and side
		ifelse(dist_ind == 3 & bear_ind == 1, 3, # far and front
		ifelse(dist_ind == 3 & bear_ind == 3, 4,  NA))))) # far and side

mod_dat$dist_ind <- as.character(mod_dat$dist_ind)

p_s <- ggplot() +
	geom_histogram(data = mod_dat, aes(x = step, group = as.factor(bin1000_ship_whale_dist), 
		colour = as.factor(bin1000_ship_whale_dist), fill = as.factor(bin1000_ship_whale_dist)),
		bins = 10, position = position_dodge(20)) +
	xlab("30s resampling: Surfacing length (m)") +
	ylab("Frequency") +
	scale_fill_discrete() +
	
	# scale_colour_manual(name = "Distance", 
		# values = c("#08677F", "#3B9AB2", "#6ECDE5"),
		# labels = c("Near", "Intermediate", "Far")) +
	# scale_fill_manual(name = "Distance", 
		# values = c("#08677F", "#3B9AB2", "#6ECDE5"),
		# labels = c("Near", "Intermediate", "Far")) +
	theme_bw()
p_s



p_t <- ggplot() +
	geom_histogram(data = mod_dat, aes(x = turn, group = as.factor(bin1000_ship_whale_dist), 
		colour = as.factor(bin1000_ship_whale_dist), fill = as.factor(bin1000_ship_whale_dist)),
		bins = 10, position = position_dodge(20)) +
	xlab("30s resampling: Surfacing length (m)") +
	ylab("Frequency") +
	scale_fill_discrete() +
	theme_bw()
p_t


n_pts <- nrow(mod_dat)
ind <- mod_dat$ID_new
nind <- length(unique(mod_dat$ID_new))

l <- (mod_dat$step)/1000
theta <- mod_dat$turn
dist_ind <- mod_dat$dist_ind
bear_ind <- mod_dat$bear_ind
comb_ind <- mod_dat$comb_ind

beh_ind <- mod_dat$beh_ind
n_pts_beh <- length(beh_ind)


kp_dist <- which(dist_ind == 1 | dist_ind == 3)
dist_ind_sm <- dist_ind[kp_dist]
dist_ind_sm[dist_ind_sm == 3] <- 2
l_dist <- l[kp_dist]
theta_dist <- theta[kp_dist]
n_pts_dist <- length(dist_ind_sm)


kp_bear <- which(bear_ind !=2)
bear_ind_sm <- bear_ind[kp_bear]
bear_ind_sm[bear_ind_sm == 3] <- 2
l_bear <- l[kp_bear]
theta_bear <- theta[kp_bear]
n_pts_bear <- length(bear_ind_sm)


kp_comb <- which(!is.na(comb_ind))
comb_ind_sm <-comb_ind[kp_comb]
l_comb <- l[kp_comb]
theta_comb <- theta[kp_comb]
n_pts_comb <- length(comb_ind_sm)