...
r_emp = function(data, mids.lower.upper = "upper", b = 500, nout){
    n = length(data)
    a.r = runif(nout,0,n)
    a.h = hist(data, plot = FALSE, breaks = b)
...
###  Turn angles
samp.turn <- function(data, mids.lower.upper = "mids", b = 100, nout){
    n = length(data)
    a.r = runif(nout,0,n)
    a.h = hist(data, plot = FALSE, breaks = b)
    cumsum.bin = cumsum(a.h$counts)
...
###  To run the above
    ...
    create.rand <- function(data, as.deg=FALSE, turn = F){
          tmp1 <- r_emp(data$dist)
          if(turn == T){
            tmp2 <- samp.turn(data$turn.angle, mids.lower.upper = "mids",
                            length.out = 25)
            tmp3 <- destination(data$x, data$y, tmp1, tmp2, as.deg = F)
          }else(tmp3 <- destination(data$x, data$y, tmp1, 
                          runif(nrow(data), -3.14, 3.14), as.deg = F) )
